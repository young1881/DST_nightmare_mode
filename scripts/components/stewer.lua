    ...
    if self.done then
        ...
        if self.product ~= nil then
            ...
            if loot ~= nil then
                ...
                -- local extraLootChance = 0
                -- if harvester ~= nil and harvester:HasTag("professionalchef") then
                --     extraLootChance = 0.1
                --     if self.inst:HasTag("mastercookware") then
                --         extraLootChance = extraLootChance * 2
                --     end
                -- end

                if harvester ~= nil and harvester.components.inventory ~= nil then
                    harvester.components.inventory:GiveItem(loot, nil, self.inst:GetPosition())
                    -- if math.random() < extraLootChance then
                    --     harvester.components.inventory:GiveItem(loot, nil, self.inst:GetPosition())
                    -- end
                else
