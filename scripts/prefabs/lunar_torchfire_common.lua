local function CreateLunarLight()
    ...
    inst.Light:SetColour(35 / 255, 45 / 255, 250 / 255)
    inst.Light:SetFalloff(TUNING.TORCH_FALLOFF[3])
    inst.Light:SetRadius(TUNING.TORCH_RADIUS[3])
...
local function MakeLunarTorchFire(name, customassets, customprefabs, common_postinit, master_postinit)
    local assets =
    {
        Asset("SCRIPT", "scripts/prefabs/lunar_torchfire_common.lua"),
    }
    ...
    local function fn()
        ...
        inst.SoundEmitter:PlaySound("rifts3/wagpunk_armor/wagpunk_armor_hat_lp","wagpunkambient_hat")
        inst.SoundEmitter:SetParameter("wagpunkambient_hat", "param00", 2)

        -- inst.SoundEmitter:PlaySound("dontstarve/wilson/torch_LP", "torch")
        -- inst.SoundEmitter:SetParameter("torch", "intensity", 1)

        inst._light = CreateLunarLight()
        inst._light.entity:SetParent(inst.entity)

        inst.OnRemoveEntity = OnRemoveEntity

        inst._lightrange = net_tinybyte(inst.GUID, "lunar_torch._lightrange", "lightrangedirty")
        if not TheWorld.ismastersim then
            inst:ListenForEvent("lightrangedirty", OnLightRangeDirty)
        else
            inst.SetLightRange = SetLightRange
        end
        inst._lightrange:set(4) -- 默认范围
