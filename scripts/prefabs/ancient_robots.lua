...
local SPIDER_ROBOT_FIRST_REVIVE_TIME = 30
local SPIDER_ROBOT_REVIVE_TIME = 90

local function revive(inst, data)
    if data ~= nil and data.name == "revive" then
        inst._roge_skip_death_loot = nil
...
local function Ondeath(inst)
    if not inst._roge_first_revive_pending and math.random() < 0.25 then
        inst._death = true
        return
    end
    inst:ListenForEvent("timerdone", revive)
    local revive_time = SPIDER_ROBOT_REVIVE_TIME
    if inst._roge_first_revive_pending then
        revive_time = SPIDER_ROBOT_FIRST_REVIVE_TIME
        inst._roge_first_revive_pending = nil
    end
    if not inst.components.timer:TimerExists("revive") then
        inst.components.timer:StartTimer("revive", revive_time)
    end
end
