...
-- 合法夜视（WX78 芯片 / 夜视果等走 PushForcedNightVision 栈）
-- 鹰眼若只调 ForceNightVision(false) 会关显示但不清栈，芯片无法再拉起 → 须类级拦截 + 失步修复
local function NightmareSyncLegitNightVisionFlag(playervision)
    local stack = playervision.forcednightvisionstack
    playervision.islegitnightvision = stack ~= nil and #stack > 0
end

local function NightmareHasLegitNightVisionStack(playervision)
    local stack = playervision ~= nil and playervision.forcednightvisionstack or nil
    return stack ~= nil and #stack > 0
end

AddClassPostConstruct("components/playervision", function(PlayerVision)
    if PlayerVision == nil or PlayerVision._nightmare_legit_nv_patched then
        return
    end
    PlayerVision._nightmare_legit_nv_patched = true

    local _ForceNightVision = PlayerVision.ForceNightVision
    function PlayerVision:ForceNightVision(force)
        if not force and NightmareHasLegitNightVisionStack(self) then
            self.islegitnightvision = true
            return
        end
        _ForceNightVision(self, force)
        if not force then
            self.islegitnightvision = false
        end
    end

    local _PushForcedNightVision = PlayerVision.PushForcedNightVision
    function PlayerVision:PushForcedNightVision(source, priority, customcctable, blend, customambienttable)
        _PushForcedNightVision(self, source, priority, customcctable, blend, customambienttable)
        NightmareSyncLegitNightVisionFlag(self)
    end

    local _PopForcedNightVision = PlayerVision.PopForcedNightVision
    function PlayerVision:PopForcedNightVision(source)
        _PopForcedNightVision(self, source)
        NightmareSyncLegitNightVisionFlag(self)
    end
end)
...
if GetModConfigData("a") then
    ...
    local function F(E)
        local pv = E ~= nil and E.components ~= nil and E.components.playervision or nil
        if pv == nil then
            return
        end
        if NightmareHasLegitNightVisionStack(pv) then
            pv.islegitnightvision = true
            if not pv.forcenightvision then
                pv:ForceNightVision(true)
            end
            return
        end
        if pv.forcenightvision and not pv.islegitnightvision then
