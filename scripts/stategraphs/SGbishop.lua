...
-- 完好主教 bishop：旧版 atk + bishop_charge 飞弹；破损 bishop_nightmare：新版瞄准 + bishop_charge2_fx
local function NightmareBishopUsesNewWeapon(inst)
	...
	if inst.prefab == "bishop_nightmare" or inst.kind == "_nightmare" then
		return true
	end
	...
	return anim ~= nil and anim:GetBuild() == "bishop_nightmare"
end
...
-- 出弹后立刻 idle；StartAttack 仍在 onenter（与 1s 周期对齐）。不用 SetTimeout 与出弹同帧抢跑。
local function NightmareBishopLegacyAttackFire(inst)
	...
	if inst.sg:HasStateTag("attack") then
		if inst._nm_bishop_attack_count ~= nil then
...
local events=
{
	...
	CommonHandlers.OnAttacked(nil, math.huge), 
	...
	EventHandler("doattack", function(inst, data)
		...
		if inst.sg:HasAnyStateTag("stunned", "electrocute", "frozen") then
			return
		end
		if not NightmareBishopUsesNewWeapon(inst) and inst.sg:HasStateTag("attack") then
			return
		end
		if inst.sg.currentstate ~= nil and inst.sg.currentstate.name == "taunt" then
			return
		end
		if not inst.sg:HasStateTag("busy") then
			inst.sg:GoToState(NightmareBishopGetAttackState(inst), data and data.target)
		end
	end),
}
...
local states=
{
   ...
   State{
	...
	-- 完好主教：旧版 bishop_charge；动画用 atk2_*（bishop_build 无 atk）
	State{
		name = "attack",
		tags = { "attack", "busy" },

		onenter = function(inst, target)
			local fire_fr = TUNING.BISHOP_LEGACY_ATTACK_FIRE_FRAME or 24
			local windup_fr = TUNING.BISHOP_LEGACY_ATTACK_WINDUP_FRAMES or 45
			local anim_sync = fire_fr / windup_fr
			local cycle = TUNING.BISHOP_LEGACY_ATTACK_PERIOD or 1.5
			...
			inst.AnimState:SetDeltaTimeMultiplier(anim_sync)
			inst.components.combat:StartAttack()
			inst.sg:SetTimeout(cycle + 4 * FRAMES)
		end,
		...
		timeline =
		{
			TimeEvent(0, function(inst) inst.SoundEmitter:PlaySound(inst.soundpath.."charge") end),
			TimeEvent((15 / ((TUNING.BISHOP_LEGACY_ATTACK_FIRE_FRAME or 24) / (TUNING.BISHOP_LEGACY_ATTACK_WINDUP_FRAMES or 45))) * FRAMES, function(inst)
				inst.SoundEmitter:PlaySound(inst.soundpath.."shoot")
			end),
			TimeEvent((TUNING.BISHOP_LEGACY_ATTACK_WINDUP_FRAMES or 45) * FRAMES, function(inst)
				if inst.sg:HasStateTag("attack") then
					NightmareBishopLegacyAttackFire(inst)
				end
			end),
	...
	-- 破损主教 bishop_nightmare：新版瞄准 + bishop_charge2_fx
	State{
		name = "attack2",
		tags = { "attack", "busy" },
