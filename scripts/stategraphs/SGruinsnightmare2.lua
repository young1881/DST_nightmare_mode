...
local events =
{
    EventHandler("attacked", function(inst, data)
        if not (inst.sg:HasAnyStateTag("attack", "hit", "noattack") or inst.components.health:IsDead()) then
            local possessed = inst.Poss2 ~= nil and inst.Poss2.Level ~= nil and inst.Poss2.Level > 0
            inst.sg:GoToState((not possessed and math.random() <= TUNING.RUINSNIGHTMARE_HORNATTACK_CHANCE) and "horn_attack" or "hit", data ~= nil and data.attacker or nil)
        end
    end),
...
local function IsPossessed(inst)
    return inst.Poss2 ~= nil and inst.Poss2.Level ~= nil and inst.Poss2.Level > 0
end
    ...
    State{
        ...
        events =
        {
            EventHandler("animover", function(inst)
                ...
                if inst.sg.statemem.target == nil or not inst.sg.statemem.target:IsValid() or inst.components.health:IsDead() then
                    ForceVisible(inst)
                    inst.sg:GoToState("appear")
                    return
                end
...
return StateGraph("ruinsnightmare", states, events, "appear", actionhandlers)
