...
if is_test then
    name = zh_en("100档噩梦模式(测试服)", "RR Collection Rework (Test)")
    icon = "modicon_test.tex"
    icon_atlas = "modicon_test.xml"
else
    name = zh_en("100档噩梦模式(正式服)", "RR Collection Rework")
    icon = "modicon.tex"
...
configuration_options = {
    ...
    { name = 'roge', label = zh_en('是否启用骰子肉鸽模式(roge模块)', 'Enable dice roguelike module (roge folder)'), options = { { description = 'Yes', data = true }, { description = 'No', data = false } }, default = true },
    ...
    { name = 'wardrobe', label = zh_en('是否启用衣柜重置技能点', 'Modify skill tree by wardrobe'), options = { { description = 'Yes', data = true }, { description = 'No', data = false } }, default = true },
    { name = 'drownable', label = zh_en('是否开启地下可以卡虚空', 'Enable walking in the void of cave'), options = { { description = 'Yes', data = true }, { description = 'No', data = false } }, default = false },
}
