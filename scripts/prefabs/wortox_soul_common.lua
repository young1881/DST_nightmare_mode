...
local function DoHeal(inst)
    ...
    if healtargetscount > 0 then
        ...
        local amt = math.max(TUNING.WORTOX_SOULHEAL_MINIMUM_HEAL,
            (TUNING.HEALING_MED * (inst.soul_heal_premult or 1) - loss_per_player * (healtargetscount - 1)) *
            (inst.soul_heal_mult or 1))
        ...
        for i = 1, healtargetscount do
            ...
            if v:HasTag("gluttony_cursed") then
                print(">>> GLUTTONY CURSE: Reducing heal for", v)
                adjusted_amt = adjusted_amt * 0.5
            end
    ...
    if sanitytargetscount > 0 then
        ...
        for i = 1, sanitytargetscount do
            ...
            local adjusted_amt = v.wortox_inclination == "nice" and amt_nice or
            v.wortox_inclination == "naughty" and amt_naughty or amt
