...
-- 与 SGruinseyeturrent.lua 中 SHADOWEYETURRET_ATTACK_CYCLE_SEC 保持一致
...
local function triggerlight(inst)
	...
	local gem = inst.colours ~= nil and inst.colours[inst.gemindex or 1] or nil
	if gem ~= nil and GEMCOLOUR[gem] ~= nil then
		inst.Light:SetColour(unpack(GEMCOLOUR[gem]))
	end
	inst.Light:SetIntensity(0.85)
	if inst._triggerlight_fade_task ~= nil then
		inst._triggerlight_fade_task:Cancel()
	end
	inst._triggerlight_fade_task = inst:DoTaskInTime(0.15, function(e)
		e._triggerlight_fade_task = nil
		if e:IsValid() and e.Light ~= nil then
			e.Light:SetIntensity(0.3)
		end
	end)
end
...
-- 发条主教瞄准线（与 modmain AddStategraphPostInit("eyeturret") 逻辑一致；暗影眼球用 SGruinseyeturrent 故在此绑定）
local function ShadowEyeUpdateBishopReticle(inst)
...
local function ShadowEyeClearBishopReticle(inst)
    if not TheWorld.ismastersim then
        return
    end
    ...
    purple = function(inst, target, damageredirecttarget)
        ...
        for i, prefab in ipairs(nightmare_prefabs) do
            ...
            if prefab == "ruinsnightmare" and nightmare ~= nil then
                ...
                nightmare.components.health:SetMaxHealth(450)
            ...
            local summoned = nightmare
            if summoned ~= nil then
                summoned:DoTaskInTime(30, function()
                    if summoned:IsValid()
                        and summoned.components.health ~= nil
                        and not summoned.components.health:IsDead() then
...
local function OnDeath(inst)
    ShadowEyeClearBishopReticle(inst)
...
local function CommonFn(types, aggro)
    ...
    inst:AddTag("monster")        -- 让MOD识别为可附身
    inst:AddComponent("locomotor") -- 最关键！让MOD能控制它
    ...
    inst.ShadowEyeUpdateBishopReticle = ShadowEyeUpdateBishopReticle
    inst.ShadowEyeClearBishopReticle = ShadowEyeClearBishopReticle
    ...
    inst.components.combat:SetAttackPeriod(SHADOWEYETURRET_ATTACK_CYCLE_SEC)
    ...
    inst:SetStateGraph("SGruinseyeturrent")
    inst:SetBrain(brain)
