TUNING.DUMBBELL_CONSUMPTION_BLUEGEM = 0.1
...
local function ConsumeMightinessOnThrow(attacker, consume_value)
    if not attacker or 
       attacker:HasTag("playerghost") or 
       not attacker.components.mightiness then
        return
    end

    local new_mightiness = math.max(0, attacker.components.mightiness.current - consume_value)
    attacker.components.mightiness.current = new_mightiness
    
    print(string.format("沃尔夫冈投掷哑铃消耗%d肌肉，当前肌肉值：%.1f", consume_value, new_mightiness))

    attacker:PushEvent("mightiness_statechange", {
        state = attacker.components.mightiness:GetState()
    })
end
...
local function onthrown(inst)
    ...
    if attacker then
        ...
        ConsumeMightinessOnThrow(attacker, 6)   --消耗
...
local function OnThrownHit(inst, attacker, target)
    ...
    local lunar_fire_duration = 8

    if inst.isiceattack then
        local x, y, z = inst.Transform:GetWorldPosition()
        local end_time = GetTime() + lunar_fire_duration

        local ring_fx = SpawnPrefab("crabking_ring_fx")
        ring_fx.Transform:SetPosition(x, 0, z) 
        ring_fx.Transform:SetScale(0.6, 0.6, 0.6) 

        local FX_COUNT = 1
        local FX_LIFETIME = 1.0
        local FX_SPAWN_INTERVAL = 0.3

        local flame_pool = {}

        local function SpawnIceFX()
            if GetTime() >= end_time then
                for _, fx in ipairs(flame_pool) do
            ...
            for i = 1, FX_COUNT do
                local fx = table.remove(flame_pool)
                if not fx or not fx:IsValid() then
                    fx = SpawnPrefab("warg_mutated_breath_fx")
                    if fx == nil then return end

                    fx.Transform:SetScale(1, 1, 1)

                    MakeLargeBurnable(fx, 6 + math.random() * 6)

                    --Remove the default handlers that toggle persists flag
                    fx.components.burnable:SetOnIgniteFn(nil)
                    fx.components.burnable:SetOnExtinguishFn(fx.Remove)
                    fx.components.burnable:Ignite()
                end

                local angle = math.random() * 2 * PI
                local speed = 1.5 + math.random() * 1.0
                local y_speed = 4 + math.random() * 1.5
                ...
                if fx.Physics then
                    fx.Physics:SetVel(speed * cosangle, y_speed, speed * -sinangle)
                end

                if fx.AnimState then
                    fx.AnimState:SetDeltaTimeMultiplier(0.7)
                end

                fx:DoTaskInTime(FX_LIFETIME, function()
                    if fx:IsValid() then
                        fx.Transform:SetScale(0.8, 0.8, 0.8)
                        fx:DoTaskInTime(0.3, function()
                            if fx:IsValid() then
                                table.insert(flame_pool, fx)
                            end
                        end)
                    end
                end)
            ...
            inst:DoTaskInTime(FX_SPAWN_INTERVAL, SpawnIceFX)
        end
        ...
        inst:DoTaskInTime(0.4, function()
            ...
            attacker:DoTaskInTime(0.1, function()
                local shou = attacker.replica.inventory and
                    attacker.replica.inventory:GetEquippedItem(EQUIPSLOTS.HANDS)
                if shou == nil then
    ...
    local AOE_ATTACK_MUST_TAGS = { "_combat" }
    local AOE_ATTACK_NO_TAGS = { "companion" }

    local x, y, z = inst.Transform:GetWorldPosition()
    local ents = TheSim:FindEntities(x, y, z, 4.1, AOE_ATTACK_MUST_TAGS, AOE_ATTACK_NO_TAGS)    --aoe范围
    ...
    for i, ent in ipairs(ents) do
        ...
        if CanDamage(inst, ent, attacker) then
            if attacker ~= nil and attacker:IsValid() then
                ...
                ent.components.combat:GetAttacked(attacker,
                    inst.components.weapon.damage(inst, inst.components.complexprojectile.attacker, ent))
            end
            canfreeze = true
        elseif attacker == ent then
            canfreeze = true
        end
        if canfreeze and inst.isiceattack and ent.components.freezable ~= nil and ent:IsValid() then
            ent.components.freezable:AddColdness(1000)   --冰冻效率
        end
    end
...
local function MakeTossable(inst)
    if inst.components.complexprojectile == nil then
        ...
        inst.components.complexprojectile:SetHorizontalSpeed(15)  --哑铃的速度
...
local function UpdateImages(inst, range)
    ...
    if inst.components.inventoryitem.owner ~= nil and inst.components.equippable ~= nil and inst.components.equippable:IsEquipped() then
        inst.components.inventoryitem.owner.AnimState:OverrideSymbol("swap_object", inst.swap_dumbbell,
            inst.swap_dumbbell_symbol)
    end
...
return
    MakeDumbbell("dumbbell", TUNING.DUMBBELL_CONSUMPTION_ROCK,
        { TUNING.DUMBBELL_EFFICIENCY_MED, TUNING.DUMBBELL_EFFICIENCY_MED, TUNING.DUMBBELL_EFFICIENCY_LOW },
        TUNING.DUMBBELL_DAMAGE_ROCK, "wolfgang1/dumbbell/stone_impact"),
    MakeDumbbell("dumbbell_golden", TUNING.DUMBBELL_CONSUMPTION_GOLD,
        { TUNING.DUMBBELL_EFFICIENCY_HIGH, TUNING.DUMBBELL_EFFICIENCY_MED, TUNING.DUMBBELL_EFFICIENCY_LOW },
        TUNING.DUMBBELL_DAMAGE_GOLD, "wolfgang1/dumbbell/gold_impact"),
    MakeDumbbell("dumbbell_marble", TUNING.DUMBBELL_CONSUMPTION_MARBLE,
        { TUNING.DUMBBELL_EFFICIENCY_HIGH, TUNING.DUMBBELL_EFFICIENCY_HIGH, TUNING.DUMBBELL_EFFICIENCY_MED },
        TUNING.DUMBBELL_DAMAGE_MARBLE, "wolfgang1/dumbbell/stone_impact", TUNING.DUMBBELL_SLOW_MARBEL),
    MakeDumbbell("dumbbell_gem", TUNING.DUMBBELL_CONSUMPTION_GEM,
        { TUNING.DUMBBELL_EFFICIENCY_HIGH, TUNING.DUMBBELL_EFFICIENCY_HIGH, TUNING.DUMBBELL_EFFICIENCY_HIGH },
        TUNING.DUMBBELL_DAMAGE_GEM, "wolfgang1/dumbbell/gem_impact"),
    MakeDumbbell("dumbbell_heat", TUNING.DUMBBELL_CONSUMPTION_HEAT,
        { TUNING.DUMBBELL_EFFICIENCY_MED, TUNING.DUMBBELL_EFFICIENCY_MED, TUNING.DUMBBELL_EFFICIENCY_LOW },
        TUNING.DUMBBELL_DAMAGE_HEAT, "wolfgang1/dumbbell/gem_impact"),
    MakeDumbbell("dumbbell_redgem", TUNING.DUMBBELL_CONSUMPTION_REDEM,
        { TUNING.DUMBBELL_EFFICIENCY_HIGH, TUNING.DUMBBELL_EFFICIENCY_HIGH, TUNING.DUMBBELL_EFFICIENCY_HIGH },
        TUNING.DUMBBELL_DAMAGE_REDGEM, "wolfgang1/dumbbell/gem_impact"),
    MakeDumbbell("dumbbell_bluegem", TUNING.DUMBBELL_CONSUMPTION_BLUEGEM,
        { TUNING.DUMBBELL_EFFICIENCY_HIGH, TUNING.DUMBBELL_EFFICIENCY_HIGH, TUNING.DUMBBELL_EFFICIENCY_HIGH },
        TUNING.DUMBBELL_DAMAGE_BLUEGEM, "wolfgang1/dumbbell/gem_impact")