...
AddPrefabPostInit("minotaur",function(inst)
    ...
    if inst.components.sleeper then
        inst.components.sleeper = nil  -- 直接解除引用
    end
    ...
    inst:ListenForEvent("death", function(m)
        if m.shadow ~= nil and m.shadow:IsValid() then
            m.shadow:Remove()
        end
        m.shadow = nil
    end)
...
State{
        ...
        events=
        {
            EventHandler("animover", function(inst)
                ...
                inst.components.timer:StartTimer("endstun", 15)  --原25s眩晕
...
local chest_loot =
{
    ...
    {item = {"thulecite"}, count = { 12,20 }},
    {item = {"thulecite_pieces"}, count = {20, 39 }},
...
function SpawnMinotaurStyleChest(inst)
    if not TheWorld.ismastersim or inst == nil or not inst:IsValid() then
        return
    end
    local spawner = SpawnPrefab("minotaurchestspawner")
    if spawner ~= nil then
        local x, y, z = inst.Transform:GetWorldPosition()
        spawner.Transform:SetPosition(x, 0, z)
    end
end
