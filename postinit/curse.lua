...
local SLOTH_GROGGINESS        = 0.6
local SLOTH_DURATION          = 0.6

local PRIDE_DAMAGE_MULT       = 1.2

local ENVY_INTERVAL           = 6
local ENVY_HEALTH_DRAIN       = 15
local ENVY_SANITY_DRAIN       = 15
local ENVY_RADIUS             = 6
local ENVY_SANITY_EXCLUDE_TAG = "lunar_aligned"

local WRATH_WEAR_MULT         = 1.5
local WRATH_DURATION          = 10

local GREED_STEAL_MIN         = 1
local GREED_STEAL_MAX         = 3

local GLUTTONY_HUNGER_PERCENT = 0.05
local GLUTTONY_ABSORB_MULT    = 0.7
local GLUTTONY_DURATION       = 20

local LUST_DAMAGE_REDUCE      = 0.75
...
CURSES                        = {
    ...
    ["嫉妒"] = {
        ...
        apply = function(inst)
            local task
            task = inst:DoPeriodicTask(ENVY_INTERVAL, function()
                ...
                local ents = TheSim:FindEntities(x, y, z, ENVY_RADIUS, { "player" }, { "playerghost" })
                local total_heal = 0
    ...
    ["暴食"] = {
        ...
        get_details = function()
            return string.format("命中时减少目标 %.1f%% 的饱食度，并使灵魂和食物恢复效果降低 %.0f%%，持续 %d 秒",
                GLUTTONY_HUNGER_PERCENT * 100, (1 - GLUTTONY_ABSORB_MULT) * 100, GLUTTONY_DURATION)
        end,
        ...
        apply = function(inst)
            inst:ListenForEvent("onhitother", function(inst, data)
                ...
                if target.components.hunger then
                    local max_hunger = target.components.hunger.max or 100
                    local drain_amount = max_hunger * GLUTTONY_HUNGER_PERCENT
                    
                    target.components.hunger:DoDelta(-drain_amount)
                end
