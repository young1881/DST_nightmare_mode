        ...
        timeline =
        {
            ...
            TimeEvent(14 * FRAMES, function(inst)
                inst.SoundEmitter:PlaySound(
                    "dontstarve_DLC001/creatures/moose/preen_feathers")
            end),
        },
    },
    ...
    State {
        ...
        timeline =
        {
            ...
            TimeEvent(15 * FRAMES, function(inst)
                ...
                for _, player in ipairs(players) do
                    if player and not player:HasTag("playerghost") then
                        ...
                        local knockback_dir = Vector3(px - x, 0, pz - z):Normalize()
                        local knockback_strength = 5

                        player:PushEvent("knockback",
                            { knocker = inst, radius = knockback_strength, prop_knockback = knockback_dir })
                    end
                end
                local num_lightnings = math.random(1, 4)

                for k = 0, num_lightnings do
                    if #players > 0 then
                        local player = players[math.random(#players)]
                        local px, py, pz = player.Transform:GetWorldPosition()
                        local pt = Vector3(px, py, pz)
                        local rad = math.random(3, 15)
                        local angle = k * 4 * PI / num_lightnings
                        local pos = pt + Vector3(rad * math.cos(angle), 0, rad * math.sin(angle))
                        inst:DoTaskInTime(0.3 * k, function()
            ...
            TimeEvent(20 * FRAMES, function(inst)
                ...
                for _, player in ipairs(players) do
                    if player:IsValid() and player.components.combat ~= nil then
                        DisarmTarget(inst, player)
                    end
                end

                local exempt_player = nil
                if num_players > 3 then
                    exempt_player = players[math.random(1, num_players)]
                end

                for _, player in ipairs(players) do
                    if player:IsValid() and player.components.combat ~= nil and player ~= exempt_player then
                        doprojectilehit(inst, inst, player)
                    end
                end
            end),

            TimeEvent(32 * FRAMES, function(inst) PlayFootstep(inst) end),
            TimeEvent(32 * FRAMES, function(inst) PlayFootstep(inst) end),
        },
        ...
        timeline =
        {
            ...
            TimeEvent(50 * FRAMES,
                function(inst)
                    ...
                    local offset = FindWalkableOffset(inst:GetPosition(), math.random() * 2 * math.pi, 4, 12) or
                        Vector3(0, 0, 0)
...
CommonStates.AddCombatStates(states,
    {
        attacktimeline =
        {
            ...
            TimeEvent(13 * FRAMES,
                function(inst) inst.SoundEmitter:PlaySound("dontstarve_DLC001/creatures/moose/swhoosh") end),
            TimeEvent(19 * FRAMES, function(inst) PlayFootstep(inst) end),
            TimeEvent(20 * FRAMES, function(inst)
                if not inst.components.timer:TimerExists("DisarmCooldown") then
                    inst.components.timer:StartTimer("DisarmCooldown", 8)
                end
