...
STRINGS.RECIPE_DESC.ROPE2_BUNCH = "一大捆绳子！"
...
RegisterInventoryItemAtlas("images/inventoryimages/brain_coral.xml", "brain_coral.tex")
...
recipes.shroomcake.test = function(cooker, names, tags)
    local mushroom_count = 0
    if names.moon_cap then mushroom_count = mushroom_count + names.moon_cap end
    if names.red_cap then mushroom_count = mushroom_count + names.red_cap end
    if names.blue_cap then mushroom_count = mushroom_count + names.blue_cap end
    if names.green_cap then mushroom_count = mushroom_count + names.green_cap end

    return mushroom_count >= 4
end
