...
local wudi_duration = 1.4   --无敌盾持续时间
local cooldown_duration = 3 --这段时间内无敌盾不会重复触发
...
AddComponentPostInit("health", function(self)
    ...
    self.AddRegenSource = function(self, source, amount, period, key, ...)
        if self.inst:IsValid() and 
           self.inst.prefab == "woodie" and 
           self.inst.components.skilltreeupdater and 
           key == "weremoose_skill" and 
           activeskill(self.inst, "woodie_allegiance_shadow") then
        ...
        if inst.sg.statemem.ismoose then
            if inst.sg.statemem.ismoosesmash then
                if activeskill(inst, "woodie_allegiance_shadow") then
                    TUNING.SKILLS.WOODIE.MOOSE_SMASH_DAMAGE = 34 * 4 + 50
                    TUNING.SKILLS.WOODIE.MOOSE_SMASH_PLANAR_DAMAGE = 100
                    -- 恐慌效果
                ...
                elseif activeskill(inst, "woodie_allegiance_lunar") then
                    ...
                    if inst:IsValid() then
                        ...
                        DoAOEWork(inst, new_x, new_z, isocean,50)
                    end
                end
            end
        end
...
------------ 鹿人三连击可续接 / 不可打断 + 击杀恢复变身值 -----------
local WERENESS_KILL_GAIN = 3

local function FixMooseComboAnimation(inst)
    if not inst:HasTag("weremoose") or not inst.sg.statemem.ismoose or inst.sg.statemem.ismoosesmash then
        return
    end
    local combo = inst._wd_moose_combo or 0
    if combo <= 0 then
        return
    end
    if inst.AnimState:IsCurrentAnimation("punch_b") or inst.AnimState:IsCurrentAnimation("moose_slam") then
        return
    end
    if combo == 1 then
...
local function UpdateMooseComboCounter(inst)
    if not inst:HasTag("weremoose") or not inst.sg.statemem.ismoose then
        return
    end
    if inst.sg.statemem.ismoosesmash then
        inst._wd_moose_combo = 0
    elseif inst.AnimState:IsCurrentAnimation("punch_b") then
        inst._wd_moose_combo = 2
    elseif inst.AnimState:IsCurrentAnimation("punch_c") then
        inst._wd_moose_combo = 1
    elseif inst.AnimState:IsCurrentAnimation("punch_a") then
        inst._wd_moose_combo = 1
    end
end

local function ResetMooseCombo(inst)
    inst._wd_moose_combo = 0
end

local function OnWoodieKillForWereness(inst, data)
    ...
    if inst:HasTag("weremoose") or inst:HasTag("beaver") or inst:HasTag("weregoose") then
        inst.components.wereness:DoDelta(WERENESS_KILL_GAIN)
    end
end

local function IsWoodieMooseComboAttack(inst)
    return inst.prefab == "woodie"
        and inst.sg ~= nil
        and inst.sg:HasStateTag("attack")
        and inst.sg.statemem.ismoose
end

local function ApplyMooseComboUninterruptible(inst)
    if IsWoodieMooseComboAttack(inst) then
        inst.sg:AddStateTag("nointerrupt")
    end
end

local function WrapWilsonSgEvent(sg, eventname, block_fn)
...
local function PatchMooseComboUninterruptible(sg)
    local attack = sg.states["attack"]
    if attack == nil or attack._wd_moose_uninterrupt_patched then
        return
    end
    attack._wd_moose_uninterrupt_patched = true

    local Timeline = attack.timeline
    local smash_idx = GetStateTimelineIndex(Timeline, 7 * FRAMES)
    if smash_idx ~= nil then
        local ev = Timeline[smash_idx]
        local old_fn = ev.fn
        ev.fn = function(inst)
            old_fn(inst)
            if IsWoodieMooseComboAttack(inst) and inst.sg.statemem.ismoosesmash then
                inst.sg:AddStateTag("nointerrupt")
            end
        end
    end

    for _, eventname in ipairs({ "equip", "unequip" }) do
        local ev = attack.events[eventname]
        if ev ~= nil then
            local old_fn = ev.fn
            ev.fn = function(inst, ...)
                if IsWoodieMooseComboAttack(inst) then
    ...
    local block_combo_interrupt = IsWoodieMooseComboAttack
    WrapWilsonSgEvent(sg, "knockback", block_combo_interrupt)
    WrapWilsonSgEvent(sg, "repelled", block_combo_interrupt)
    WrapWilsonSgEvent(sg, "snared", block_combo_interrupt)
    WrapWilsonSgEvent(sg, "startled", block_combo_interrupt)
end

local function PatchMooseComboPersist(sg)
    local attack = sg.states["attack"]
    if attack == nil or attack._wd_moose_combo_patched then
        return
    end
    attack._wd_moose_combo_patched = true

    local old_onenter = attack.onenter
    attack.onenter = function(inst, ...)
        if inst.prefab == "woodie" and inst:HasTag("weremoose") and (inst._wd_moose_combo or 0) > 0 then
            if inst.components.combat ~= nil and inst.components.combat:InCooldown() then
                inst.components.combat:ResetCooldown()
            end
        end
        local ret = { old_onenter(inst, ...) }
        if inst.prefab == "woodie" then
            FixMooseComboAnimation(inst)
            UpdateMooseComboCounter(inst)
            ApplyMooseComboUninterruptible(inst)
        end
        return unpack(ret)
    end

    PatchMooseComboUninterruptible(sg)
end

AddStategraphPostInit("wilson", PatchMooseComboPersist)
...
AddPrefabPostInit("woodie", function(inst)
    ...
    inst._wd_moose_combo = 0
    inst:ListenForEvent("transform_person", ResetMooseCombo)
    inst:ListenForEvent("killed", OnWoodieKillForWereness)
