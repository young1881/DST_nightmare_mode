...
local PingWheel = Class(Widget, function(self)
	...
	local lang=TheNet:GetIsClient() and GetModConfigData("language",true) or nil
	if lang=="default" then
		lang=LOC and LOC.CurrentLocale
	end
	...
	--Chinese
	if lang=="chinese" or lang=="zh" or lang=="zht" or lang=="zhr" then
		pings = {
			{name = "omw", image = "Omw", text = "在路上"},
			{name = "gohere", image = "GoHere", text = "你去这里"},
			{name = "explore", image = "Explore", text = "探索"},
			{name = "danger", image = "Danger", text = "危险"},
		}
		specials = {
			{name = "generic", image = "", text = "集合", x = 0, y = 0},
			{name = "cancel", image = "Cancel", text = "取消", x = -specialdist, y = -specialdist},
			{name = "delete", image = "Delete", text = "删除", x = specialdist, y = specialdist},
			{name = "clear", image = "Clear", text = "清空全图", x = specialdist, y = -specialdist},
		}
	elseif lang=="portuguese" or lang=="pt" then
		--Portugese
		--no translation
	end
	--[[
