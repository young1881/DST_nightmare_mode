...
-- local function AlterGuardianPhase1_OnHitOther(inst, data)
--     if data ~= nil and data.target ~= nil then
--         data.target:PushEvent("knockback", {
--             knocker = inst,       
--             radius = 5,            
--             strengthmult = 1.5     -- 击退倍率
--         })
...
AddPrefabPostInit("alterguardian_phase1", function(inst)
    ...
    inst:ListenForEvent("onhitother", AlterGuardianPhase1_OnHitOther)
...
AddStategraphPostInit("alterguardian_phase1", function(sg)
    ...
    sg.states.roll.timeline =
    {
        TimeEvent(FRAMES, function(inst)
            ...
            local x, y, z = inst.Transform:GetWorldPosition()
            local targets = TheSim:FindEntities(x, y, z, 5, 
                {"_combat"},  
                {"INLIMBO", "playerghost", "noattack", "notarget"} 
            )
...
local function spawnbarrier(inst)
    ...
    for i = 1, number do
        ...
        if TheWorld.Map:IsPassableAtPoint(newpt.x, 0, newpt.z) then
            inst:DoTaskInTime(0.5, spawn_spike_with_pos, newpt, angle)
        end
        angle = angle + (380 / number)
    end
end
    ...
    for i, v in ipairs(ents) do
        ...
        then
            ...
            if v.components.freezable ~= nil then
                v.components.freezable:AddColdness(99, 12) --freezetime:12
            end
            if wasfrozen then
...
local function startcastspell(inst, isfreeze)
    isfreeze = true
    local x,y,z = inst.Transform:GetWorldPosition()
    local fx = SpawnPrefab("freeze_blast")
    if fx then
        fx.owner = inst  
        fx:ListenForEvent("onremove", function() removecrab(fx) end, inst)
        fx.Transform:SetPosition(x,y,z) 
        local gems = inst.countgems(inst)
        local scale = 0.75 + Remap(gems.blue,0,9,0,1.55)
...
local function endcastspell(inst, lastwasfreeze)
    ...
    local x,y,z = inst.Transform:GetWorldPosition()
    local ents = TheSim:FindEntities(x, y, z, 25, nil, nil, {"freeze_blast"})
    for i,ent in pairs(ents) do
        if ent:IsValid() then
            if not inst.components.health:IsDead() then
                if ent.owner == inst then
                    ent:DoTaskInTime(1, function() ent:PushEvent("endspell") end)
                end
            else
                ent:Remove()
            end
...
local function OnCollide(inst, other)
    if other ~= nil and other:IsValid() and other.components.workable ~= nil and
        other.components.workable:CanBeWorked() and other.components.workable.action ~= ACTIONS.NET then
...
AddPrefabPostInit("freeze_blast", function(inst)
    if not TheWorld.ismastersim then return end

    if inst.components.entitytracker then
        if inst._removetask then
            inst._removetask:Cancel()
            inst._removetask = nil
        end
        inst._removetask = inst:DoTaskInTime(5, inst.Remove)
    end
    local old_onhit = inst.components.projectile ~= nil and inst.components.projectile.onhit or nil
    if old_onhit ~= nil then
        inst.components.projectile:SetOnHitFn(function(inst, target)
            old_onhit(inst, target)
            if target ~= nil and target:IsValid() and target.components.freezable ~= nil then
                target.components.freezable:AddColdness(40) 
...
AddStategraphState("alterguardian_phase2", State {
    ...
    onenter = function(inst, target)
        ...
        inst.components.timer:StopTimer("icespell_cd")
        inst.components.timer:StartTimer("icespell_cd", 85)
        
        inst.sg.statemem.spell_complete = false
        ...
        if not inst.components.timer:TimerExists("casting_timer") then
            ...
            local cast_time = 4 
            inst.components.timer:StartTimer("casting_timer", cast_time) 
        end
    end,
    onupdate = function(inst)
        if inst.components.timer:TimerExists("casting_timer") then
            local time_left = inst.components.timer:GetTimeLeft("casting_timer")
            if time_left <= 0.1 and not inst.sg.statemem.spell_complete then
                inst.sg.statemem.spell_complete = true
            end
        end
    end,
    timeline = {
    ...
    onexit = function(inst)
        ...
        inst.sg.statemem.spell_complete = nil
    end,
    events = {
        EventHandler("timerdone", function(inst,data)
            if data.name == "casting_timer" then
                ...
                inst.sg.statemem.keepcast = false

                inst:DoTaskInTime(1.000, function()
                    inst.sg:GoToState("idle")
                end)
            end
        end),
        EventHandler("animover", function(inst)
            ...
            if inst.components.timer:TimerExists("casting_timer") then
                ...
                inst.AnimState:PushAnimation("attk_chop", true)
            end
...
local function EraserLaserTrigger(inst, delay, targets, skiptoss, skipscorch)
...
AddPrefabPostInit("alterguardian_laser", function(inst)
	...
	local prefab_trigger = inst.Trigger
	inst.Trigger = function(laser_inst, delay, targets, skiptoss, skipscorch, scale, scorchscale, hitscale, heavymult, mult, forcelanded)
		if laser_inst.type == "eraser" then
			EraserLaserTrigger(laser_inst, delay, targets, skiptoss, skipscorch)
		else
			prefab_trigger(laser_inst, delay, targets, skiptoss, skipscorch, scale, scorchscale, hitscale, heavymult, mult, forcelanded)
		end
	end
end)
