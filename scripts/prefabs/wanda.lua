...
local function becomenormal(inst, silent)
    ...
    TUNING.WANDA_READ_PENALTY = 18

    if inst.components.reader == nil then
        inst:AddComponent("reader")
        inst.components.reader:SetSanityPenaltyMultiplier(TUNING.WANDA_READING_SANITY_MULT)
...
local function redirect_to_oldager(inst, amount, overtime, cause, ignore_invincible, afflicter, ignore_absorb)
    -- 兼容性保护：对 0 点伤害（比如只是为了刷新血条 HUD 的 DoDelta(0)）
    -- 不走 oldager 重定向，避免和其它组件（护甲套装、位面伤害等）之间出现递归调用。
    if amount == 0 then
        return false
    end
...
local function ShadowWeaponFx(inst, target, damage, stimuli, weapon, damageresolved)
    if weapon ~= nil and target ~= nil and target:IsValid() and weapon:IsValid() and weapon:HasTag("shadow_item") then
        local fx_prefab = inst.age_state == "old" and
            (weapon:HasTag("pocketwatch") and "wanda_attack_pocketwatch_old_fx" or "wanda_attack_shadowweapon_old_fx")
            or
            inst.age_state == "normal" and
            (weapon:HasTag("pocketwatch") and "wanda_attack_pocketwatch_normal_fx" or "wanda_attack_shadowweapon_normal_fx")
            or nil
...
local function OnGetItem(inst, data)
    ...
    if item ~= nil and item:HasTag("pocketwatch") then
        item.components.inventoryitem.keepondeath = item.prefab ~=
            "pocketwatch_revive" -- drop the revive watch so she can haunt it
