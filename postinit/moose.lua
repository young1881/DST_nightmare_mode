...
AddPrefabPostInit("moose", function(inst)
    ...
    inst.components.lootdropper:AddChanceLoot('goose_feather', 1)
    inst.components.lootdropper:AddChanceLoot('goose_feather', 1)
    inst:DoTaskInTime(0, function(m)
        if m:IsValid() and m.components.health ~= nil and RogeScaleAdjustedMaxHealth ~= nil then
            RogeScaleAdjustedMaxHealth(m, TUNING.MOOSE_HEALTH)
        end
    end)
end)
