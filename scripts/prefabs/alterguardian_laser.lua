...
local function DoDamage(inst, targets, skiptoss, skipscorch, scale, scorchscale, hitscale, heavymult, mult, forcelanded)
    ...
    --for knockback
    local disttocaster = mult and inst.caster and inst.caster:IsValid() and
        math.sqrt(inst.caster:GetDistanceSqToPoint(x, y, z)) or nil
        ...
        if not targets[v] and v:IsValid() and not v:IsInLimbo() and not (v.components.health ~= nil and v.components.health:IsDead()) and not v:HasTag("laser_immune") then
            ...
            if dsq_to_laser < range * range then
                ...
                elseif inst.components.combat:CanTarget(v) then
                    ...
                    --for knockback
                    local strengthmult = mult and
                        ((v.components.inventory and v.components.inventory:ArmorHasTag("heavyarmor") or v:HasTag("heavybody")) and heavymult or mult) or
                        nil
...
local function Trigger(inst, delay, targets, skiptoss, skipscorch, scale, scorchscale, hitscale, heavymult, mult,
                       forcelanded)
    if inst.task ~= nil then
        inst.task:Cancel()
        if (delay or 0) > 0 then
            inst.task = inst:DoTaskInTime(delay, DoDamage, targets or {}, skiptoss or {}, skipscorch, scale, scorchscale,
                hitscale, heavymult, mult, forcelanded)
        else
            DoDamage(inst, targets or {}, skiptoss or {}, skipscorch, scale, scorchscale, hitscale, heavymult, mult,
                forcelanded)
        end
    end
end
