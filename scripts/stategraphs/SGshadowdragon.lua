    ...
    State{
        ...
        onenter = function(inst)
            ...
            if target and target:IsValid() then
                rot = inst:GetAngleToPoint(target.Transform:GetWorldPosition())
            end
        ...
        timeline=
        {
            ...
            --     TryWave2(inst, (inst.sg.statemem.rot or 0) + 360/16)
            -- end),
        ...
        {
            EventHandler("animover", function(inst)
                inst.components.timer:StartTimer("wave_cd",14)
                if math.random() < .333 then
                    inst.sg:GoToState("taunt")
                else
                    inst.sg:GoToState("idle")
