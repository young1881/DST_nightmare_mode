...
function GlobalPositions:OnLoad(data)
    ...
    local interval = 0.25
    local maxinterval = 64
    local function TryLoadingWorldMap()
        -- if TheNet:IsDedicated() then
        if TheWorld.worldmapexplorer and TheWorld.worldmapexplorer.MapExplorer:LearnRecordedMap(data.worldmap) then
            -- print("Succeeded at loading the world map.")
            self.map_loaded = true
        elseif interval < maxinterval then
            print("[GLOBALPOSITIONS]Failed to load world map, trying again...")
            interval = interval * 2
            self.inst:DoTaskInTime(interval, TryLoadingWorldMap)
        end
        -- end
    end
    -- TheWorld can't have its own map on non-dedicated servers
    if TheNet:IsDedicated() and data and data.worldmap then
        if not TheWorld.worldmapexplorer then
            print("[GLOBALPOSITIONS] Loading map, but no wme")
            -- try again
            scheduler:ExecuteInTime(0.1, TryLoadingWorldMap, "TryLoadingWorldMap")
        end
        if TheWorld.worldmapexplorer and TheWorld.worldmapexplorer.MapExplorer then
...
function GlobalPositions:AddServerEntity(inst)
    ...
    classified.name = (pingcolour and player) and (inst.name .. "\n(" .. player.name .. ")")
...
function GlobalPositions:RemoveServerEntity(inst)
    if shouldShowIndicator(self.positions[inst.GUID]) and ThePlayer and ThePlayer.userid ~= inst.userid and ThePlayer.HUD then
...
function GlobalPositions:AddClientEntity(inst)
    ...
    inst.name = (not pingcolour and player) and player.name or STRINGS.NAMES[prefabname:upper()]
                    or inst.parentprefab:value()
    if pingcolour then
