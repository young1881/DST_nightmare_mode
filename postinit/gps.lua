...
-- 韦伯：探图与地图位置仅对其他韦伯共享；可接收他人探图与位置
local WEBBER_GPS_PEER_TAG = "webber_gps_peer"

local function IsWebberPlayer(inst)
	return inst ~= nil and inst.prefab == "webber"
end

local function GetWebberMapRevealPosition(inst)
	local x, y, z = inst.Transform:GetWorldPosition()
	local poss2 = inst.Poss2
	if poss2 ~= nil and poss2.Possessing ~= nil and poss2.Possessing:IsValid() then
		x, y, z = poss2.Possessing.Transform:GetWorldPosition()
	end
	return x, y, z
end

AddComponentPostInit("maprevealer", function(self)
	local old_reveal = self.RevealMapToPlayer
	function self:RevealMapToPlayer(player, ...)
		if IsWebberPlayer(self.inst) then
			if not IsWebberPlayer(player) then
				return
			end
			if player._PostActivateHandshakeState_Server ~= POSTACTIVATEHANDSHAKE.READY
				or player.player_classified == nil then
				return
			end
			player.player_classified.MapExplorer:RevealArea(GetWebberMapRevealPosition(self.inst))
			return
		end
		return old_reveal(self, player, ...)
	end
end)
...
AddPlayerPostInit(function(inst)
	...
	if IsWebberPlayer(inst) then
		inst:AddTag(WEBBER_GPS_PEER_TAG)
