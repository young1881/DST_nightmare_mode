...
TUNING.ALTERGUARDIAN_PHASE1_SUMMONCOOLDOWN = 3.5   --召唤CD
TUNING.ALTERGUARDIAN_PHASE1_SHIELDTRIGGER = 10000 --盾触发
...
TUNING.ALTERGUARDIAN_PHASE2_SPIKEDAMAGE = 180   --尖刺攻击力
...
TUNING.ALTERGUARDIAN_PHASE3_ATTACK_PERIOD = 2.75      --攻击间隔
TUNING.ALTERGUARDIAN_PHASE3_TRAP_CD = 15             --陷阱CD
TUNING.ALTERGUARDIAN_PHASE3_SUMMONCOOLDOWN = 25      --召唤CD
TUNING.ALTERGUARDIAN_PHASE3_DAMAGE = 300             --攻击力
TUNING.ALTERGUARDIAN_PHASE3_LASERDAMAGE = 450        --激光攻击力
TUNING.ALTERGUARDIAN_PHASE3_TRAP_LANDEDDAMAGE = 120   --陷阱攻击力
TUNING.ALTERGUARDIAN_PHASE3_TARGET_DIST = 50         --仇恨范围
TUNING.ALTERGUARDIAN_PHASE3_ATTACK_RANGE = 60        --攻击范围
TUNING.ALTERGUARDIAN_PHASE3_WALK_SPEED = 17.5        --移速
TUNING.ALTERGUARDIAN_PHASE3_RUNAWAY_BLOCK_TIME = 0 --可以阻挡天体移动的时间
TUNING.ALTERGUARDIAN_PHASE3_SUMMONMAXLOOPS = 120     --天体大虚影总数
TUNING.ALTERGUARDIAN_PHASE3_SUMMONRSQ = 950         --蓝圈大小，20为1格
TUNING.ALTERGUARDIAN_PHASE3_MAX_STUN_LOCKS = 0     --天体可僵直次数


TUNING.ALTERGUARDIAN_PHASE1_MINROLLCOUNT = 8
local SupremeEpic_planar_dmg = 25 --天体织影位面伤害


--TUNING.ALTERGUARDIAN_PHASE2_SUMMONCOOLDOWN = 24.25
TUNING.ALTERGUARDIAN_PHASE2_SPIKECOOLDOWN           = 9   --地刺cd
TUNING.ALTERGUARDIAN_PHASE2_LIGHTNINGCOOLDOWN       = 30  --旋风劈cd
...
TUNING.ALTERGUARDIAN_PHASE3_TRAP_MAXRANGE           = 10  --召唤陷阱的范围
TUNING.ALTERGUARDIAN_PHASE3_TRAP_LT                 = 40 --陷阱持续时间
TUNING.ALTERGUARDIAN_PHASE3_ERASERCOOLDOWN          = 60 --秒杀cd

--只因
TUNING.STALKER_HEALTH                               = 5000  -- 复活的骨架（第8天起 roge 为 8000）
TUNING.STALKER_ATRIUM_HEALTH                        =100000 --血量
TUNING.STALKER_ATRIUM_PHASE2_HEALTH                 =75000 --二阶段
TUNING.STALKER_CHANNELERS_CD                        = 6.0   --苦笑暗影CD
TUNING.STALKER_CHANNELERS_COUNT                     = 6     --苦笑暗影数量
TUNING.STALKER_MINIONS_COUNT                        = 120   --奴隶数量
TUNING.STALKER_MINIONS_CD                           = 1.0   --奴隶CD
TUNING.STALKER_FEAST_HEALING                        = 5000  --回血
TUNING.STALKER_MINDCONTROL_CD                       = 12    --精神控制CD
TUNING.STALKER_MINDCONTROL_RANGE                    = 14    --精神控制范围
TUNING.STALKER_MINDCONTROL_DURATION                 = 3.5   --精神控制时长
...
--春鹅加强
TUNING.MOOSE_HEALTH                                 = 4000 -- 血量（第8天起 roge +50% 为 6000）
...
-- 大冰鲨
TUNING.SHARKBOI_HEALTH                              = 5000  -- 血量（第8天起 roge 为 8000）

-- 梦魇疯猪（daywalker，最终血量由 postinit/daywalker.lua 强制为 30000）
TUNING.DAYWALKER_HEALTH                             = 30000

-- 蜂后
TUNING.BEEQUEEN_HEALTH                              = 3500             -- 总血量
...
AddComponentPostInit("freezable", function(self)
	...
	self.AddColdness = function(self, ...)
		...
		if inst:HasTag("epic") and not inst:HasTag("king_squid") then return end
		return oldAddColdness(self, ...)
	end
	local oldFreeze = self.Freeze
	self.Freeze = function(self, ...)
		local inst = self.inst
		if inst:HasTag("epic") and not inst:HasTag("king_squid") then
			self.coldness = 0
			self:UpdateTint()
			return
		end
...
-- 完好主教（旧版 bishop_charge）：完整攻击周期 1 秒（30 帧蓄力 @30fps，出弹对齐动画）
TUNING.BISHOP_LEGACY_ATTACK_PERIOD = 1.5
TUNING.BISHOP_LEGACY_ATTACK_WINDUP_FRAMES = 45 -- 1.5s @ 30fps
TUNING.BISHOP_LEGACY_ATTACK_FIRE_FRAME = 24
...
AddPrefabPostInit("willow_ember", function(inst)
	...
	local old_oneaten = edible.oneatenfn
	edible:SetOnEatenFn(function(food, eater)
		if eater == nil or not eater:HasTag("player") then
			return
		end
		if old_oneaten ~= nil then
			old_oneaten(food, eater)
		end
	end)
end)
